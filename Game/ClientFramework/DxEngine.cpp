...
void DxEngine::late_Init(WindowInfo windowInfo)
{
	...
	case 2:
	{
		...
		boss2Skill.Init("../Resources/Boss2Skill.txt", ObjectType::Blend); // type¿ª πŸ≤„º≠ ¡ˆ¡§
		...
		boss2Skill.Make_SRV();
		boss2Skill.CreatePSO(L"..\\Circle.hlsl"); // ∫Ì∑ªµÂpso±∏«ˆ
		...
		for (int i = 0; i < NPCMAX; i++) //npc ÔøΩÔøΩÔøΩÔøΩ
		{
			...
			case TY_BOSS_1:
			{
				...
				for (Subset j : boss._animationPtr->mSubsets)
				{
					...
					if (count == 2) { // eye, ¡∂∞«ø° boss∫∞ type√ﬂ∞° « ø‰
						boss._tex._srvHandle = boss._tex._srvHeap->GetCPUDescriptorHandleForHeapStart();
				...
				if (particle_pos0[i].x != particles[i].pos.m128_f32[0] || particle_pos0[i].y != particles[i].pos.m128_f32[1] || particle_pos0[i].z != particles[i].pos.m128_f32[2])
				{
					for (OBJECT& player : playerArr)
					{
						if (player._on)
						{
							...
							{
								if (player._transform.z - collision_scale < particles[i].pos.m128_f32[2] && particles[i].pos.m128_f32[2] < player._transform.z + collision_scale // ¡§∏È
							...
							if (particle_pos0[i].x >= player._transform.x + collision_scale && player._transform.x + collision_scale > particles[i].pos.m128_f32[0])
							{
								if (player._transform.z - collision_scale < particles[i].pos.m128_f32[2] && particles[i].pos.m128_f32[2] < player._transform.z + collision_scale // »ƒ∏È
									&& particles[i].pos.m128_f32[1] >= 0.f && particles[i].pos.m128_f32[1] <= 1.f)
								{
									particles[i].velocity.m128_f32[0] = particles[i].velocity.m128_f32[0] * -coefficient_of_restitution;
									particles[i].pos.m128_f32[0] = particle_pos0[i].x;
							...
							{
								if (player._transform.x - collision_scale < particles[i].pos.m128_f32[0] && particles[i].pos.m128_f32[0] < player._transform.x + collision_scale // øﬁ¬ √¯∏È
									&& particles[i].pos.m128_f32[1] >= 0.f && particles[i].pos.m128_f32[1] <= 1.f)
								{
							...
							if (particle_pos0[i].z >= player._transform.z + collision_scale && player._transform.z + collision_scale > particles[i].pos.m128_f32[2])
							{
								if (player._transform.x - collision_scale < particles[i].pos.m128_f32[0] && particles[i].pos.m128_f32[0] < player._transform.x + collision_scale // ø¿∏•¬  √¯∏È
									&& particles[i].pos.m128_f32[1] >= 0.f && particles[i].pos.m128_f32[1] <= 1.f)
								{
									particles[i].velocity.m128_f32[2] = particles[i].velocity.m128_f32[2] * -coefficient_of_restitution;
									particles[i].pos.m128_f32[2] = particle_pos0[i].z;
							...
							{
								if (player._transform.x - collision_scale < particles[i].pos.m128_f32[0] && particles[i].pos.m128_f32[0] < player._transform.x + collision_scale // ¿≠∏È
