...
void AnimationObjectLoader::LoadVertexAnimationObjectForPillar(const string& filename, vector<Vertex>& vertices, vector<UINT>& indices, vector<Subset>& subsets, vector<int>& mBoneHierarchy, vector<XMFLOAT4X4>& mBoneOffsets, vector<vector<vector<Keyframe>>>& animations)
{
	ifstream fin(filename);
