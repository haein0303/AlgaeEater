...
public:
	...
	void CreateAnimationVertexBuffer(ComPtr<ID3D12Resource>& vertexBuffer, D3D12_VERTEX_BUFFER_VIEW& vertexBufferView, const vector<SkinnedVertex>& buffer, shared_ptr<Device> devicePtr)
	{
		UINT bufferSize = static_cast<UINT>(buffer.size()) * sizeof(SkinnedVertex);
