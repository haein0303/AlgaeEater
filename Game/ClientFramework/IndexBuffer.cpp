...
void IndexBuffer::CreateIndexBuffer(const vector<UINT>& buffer, shared_ptr<Device> devicePtr, int isPlayer) {
	...
	{
		devicePtr->_device->CreateCommittedResource(&heapProperty, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&_playerIndexBuffer));

		void* indexDataBuffer = nullptr;
		CD3DX12_RANGE readRange(0, 0);
		_playerIndexBuffer->Map(0, &readRange, &indexDataBuffer);
		memcpy(indexDataBuffer, &buffer[0], bufferSize);
		_playerIndexBuffer->Unmap(0, nullptr);

		_playerIndexBufferView.BufferLocation = _playerIndexBuffer->GetGPUVirtualAddress();
		_playerIndexBufferView.Format = DXGI_FORMAT_R32_UINT;
		_playerIndexBufferView.SizeInBytes = bufferSize;
	}
	else
	{
