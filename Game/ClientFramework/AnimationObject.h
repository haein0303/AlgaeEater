...
public:
	...
	string ClipName;
	float TimePos = 0.0f;
	float server_time_pos0 = 0.0f;
	float server_time_pos = 0.0f;
...
public:
	...
	void GetFinalTransforms(const string& clipName, float timePos, float sever_time_pos, vector<XMFLOAT4X4>& finalTransforms);
};